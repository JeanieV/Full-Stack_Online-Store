...
const fetchData = async () => {
  ...
  try {
    ...
    const tools = await Tools.fetchAll();
    console.log(tools);
    ...
    tools.forEach((tools) => {
      const productCard = generateProductCard(tools);
    ...
    readMoreButtons.forEach((button, index) => {
      button.addEventListener('click', () => {
        showModal(tools[index]); // Pass the product to showModal
      });
    });
